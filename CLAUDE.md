...
`src/pages/index.astro` is the only page. It composes `BaseLayout` → `Hero`, `Overview`, `Agenda`, `Presentations`, `Breakouts`, `Hotels`.
...
- `src/data/agenda.ts` — the `event` object (title, dates, city, venue, `registerUrl`, `registrationCloses`) and the `agenda` array of days/items. Agenda items take `time`, `title`, optional `note`, optional `locationId` (resolved against `locations.ts` to print that location's address and menu link under the note — the address is never copied into `agenda.ts`), and optional `kind` (`"session" | "break" | "social" | "breakout"` — social/break rows get a mint accent, breakout rows a coral one). General-session items also carry a `presentations` array of `{ site, org }`; `Presentations.astro` derives its side-by-side columns from those same entries, so the site order is edited here only. Day 2 breakout items carry a `breakouts` array of concurrent tracks — `code`, `title`, `leads`, a one-line `detail` for the agenda row, plus the long-form `description` and optional `goals` that `Breakouts.astro` renders as cards. Description and title live on the same entry on purpose; there is no lookup by `code`.
- `src/data/locations.ts` — the single source for every event location. Each entry has `id`, `kind` (`"venue" | "reception" | "hotel"`), `name`, `address`, `lat`, `lng`, and optional `url`/`note`/`menuUrl`. The Overview map pins **all** entries; the Hotels section renders only the `hotel` ones. Plain lat/lng — no maps API key involved. `menuUrl` points at a static file under `public/` (the Pinstripes menu PNG) and surfaces as a "View menu" link in the map legend, the map pin popup, and any agenda row pointing at that location.
- `src/data/presentations.ts` — `presentationFormat`: the Day 1 talk length, slide count, and the six `topics` every site deck covers. Array order is the slide order — `Presentations.astro` numbers the cards `01`–`06` from the index, so reordering the array renumbers them.
...
Links that open in a new tab take `src/components/ExternalIcon.astro` after the label. It takes no props and carries its own scoped sizing styles, since a parent's scoped CSS cannot reach it. The Leaflet popup in `Overview.astro` repeats the same SVG inline — its client script is a separate bundle and cannot render an Astro component — so keep the two in sync.
